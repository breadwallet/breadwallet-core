...
public class TransactionApi {
    ...
    private void getTransactionsOnExecutor(String id, List<String> addresses, long beginBlockNumber,
        ...
        for (int i = 0; i < chunkedAddressesList.size() && error[0] == null; i++) {
            ...
            for (long j = beginBlockNumber; j < endBlockNumber && error[0] == null; j += PAGINATION_COUNT) {
                ImmutableListMultimap.Builder<String, String> paramsBuilder = ImmutableListMultimap.builder();
                paramsBuilder.putAll(baseParams);
                paramsBuilder.putAll("start_height", String.valueOf(j));
                paramsBuilder.putAll("end_height", String.valueOf(Math.min(j + PAGINATION_COUNT, endBlockNumber)));
