...
public class AccountAIT {
    ...
    public void testAccountPhrase() {
        byte[] phrase = "ginger settle marine tissue robot crane night number ramp coast roast critic".getBytes(StandardCharsets.UTF_8);
        assertTrue(Account.validatePhrase(phrase, BIP39_WORDS_EN));

        phrase = Account.generatePhrase(BIP39_WORDS_EN);
        assertTrue(Account.validatePhrase(phrase, BIP39_WORDS_EN));

        phrase = "Ask @jmo for a pithy quote".getBytes(StandardCharsets.UTF_8);
        assertFalse(Account.validatePhrase(phrase, BIP39_WORDS_EN));
    }

    private static List<String> BIP39_WORDS_EN = Arrays.asList("abandon",
            "ability",
            "able",
            "about",
            "above",
