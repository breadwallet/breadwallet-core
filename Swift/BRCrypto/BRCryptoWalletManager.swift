    ...
    internal var clientBTC: BRCryptoCWMClientBTC {
        return BRCryptoCWMClientBTC (
           ...
           /*
            ...
            funcSubmitTransaction: { (context, cwm, sid, transactionBytes, transactionBytesLength, hashBytes, hashBytesLength) in
                let manager = Unmanaged<WalletManager>.fromOpaque(context!).takeUnretainedValue()
                ...
                let hash = Data (bytes: hashBytes!, count: hashBytesLength)
                manager.query.createTransaction (blockchainId: manager.network.uids, hash: hash, transaction: data) {
                    (res: Result<BlockChainDB.Model.Transaction, BlockChainDB.QueryError>) in
                    defer { cryptoWalletManagerGive (cwm!) }
                    res.resolve(
