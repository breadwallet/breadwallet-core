...
public final class SystemBase: System {
    ...
    internal func add (manager: WalletManager) {
        if !managers.contains(where: { $0 === manager }) {
            guard let manager = manager as? WalletManagerImplC
...
//            }
    ...
    internal func managerBy (core: BRCryptoWalletManager) -> WalletManagerImplC? {
        return managers
            .first {
                ($0 as? WalletManagerImplC).map { $0.core == core } ?? false
            } as? WalletManagerImplC
    }

    internal func managerBy (eth: BREthereumEWM) -> WalletManager? {
        return managers
            .first {
                ($0 as? WalletManagerImplC).map {
                    CRYPTO_TRUE == cryptoWalletManagerHasETH ($0.core, eth)
                    } ?? false
        }
    }
    internal func managerBy (btc: BRWalletManager) -> WalletManager? {
        return managers
            .first {
                ($0 as? WalletManagerImplC).map {
                    CRYPTO_TRUE == cryptoWalletManagerHasBTC($0.core, btc)
                    } ?? false
        }
    }
    ...
    public func createWalletManager (network: Network,
        ...
        let manager = WalletManagerImplC (system: self,
                                          listener: listener!,
                                          account: account,
                                          network: network,
                                          mode: mode,
...
//        precondition (nil != manager)
    ...
    internal var cryptoListener: BRCryptoCWMListener {
        ...
        return BRCryptoCWMListener (
            ...
            walletEventCallback: { (context, cwm, wid, event) in
                ...
                guard let cwm = cwm, let wid = wid
                    else { print ("SYS: Event: \(event.type): Missed {cwm, wid}"); return }

                guard let manager = system.managerBy (core: cwm),
                    let wallet   = manager.walletBy  (core: wid)
            ...
            transferEventCallback: { (context, cwm, wid, tid, event) in
                ...
                guard let cwm = cwm, let wid = wid, let tid = tid
    ...
    internal var cryptoClient: BRCryptoCWMClient {
        let this = self
        return BRCryptoCWMClient (
            context: Unmanaged<SystemBase>.passUnretained(this).toOpaque(),

            btc: BRCryptoCWMClientBTC (
                funcGetBlockNumber: { (context, bid, rid) in
                    ...
                    if let this = system.managerBy (btc: bid) {
                        system.query.getBlockNumberAsBTC (bwm: bid,
                ...
                funcGetTransactions: { (context, bid, begBlockNumber, endBlockNumber, rid) in
                    ...
                    if let manager = system.managerBy (btc: bid) {
                        ...
                        system.queue.async {
                            ...
                            repeat {
                                ...
                                system.query.getTransactionsAsBTC (bwm: bid,
                ...
                funcSubmitTransaction: { (context, bid, wid, tid, rid) in
                    ...
                    guard let manager  = system.managerBy  (btc: bid)
//                        let   wallet   = manager.walletBy  (impl: WalletImplC.Impl.bitcoin(wid: wid)),
//                        let   _        = wallet.transferBy (impl: TransferImplC.Impl.bitcoin(wid: wid, tid: tid))
                    ...
                    system.query.putTransaction (blockchainId: manager.network.uids,
            ...
            eth: BRCryptoCWMClientETH (
                funcGetBalance: { (context, coreEWM, wid, address, rid) in
                    ...
                    if let manager = system.managerBy (eth: eid) {

                        let address = asUTF8String(address!)
                        if nil == ewmWalletGetToken (eid, wid) {
                            system.query.getBalanceAsETH (ewm: eid,
                                                          wid: wid,
                                                          address: address,
                            ...
                            system.query.getBalanceAsTOK (ewm: eid,
                                                          wid: wid,
                                                          address: address,
                                                          rid: rid) { (wid, balance, rid) in
                                                            ewmAnnounceWalletBalance (eid, wid, balance, rid)
                ...
                funcGetGasPrice: { (context, coreEWM, wid, rid) in
                    ...
                    if let manager = system.managerBy (eth: eid) {
                        system.query.getGasPriceAsETH (ewm: eid,
                                                       wid: wid,
                ...
                funcEstimateGas: { (context, coreEWM, wid, tid, from, to, amount, data, rid)  in
                    ...
                    if let manager = system.managerBy (eth: eid) {
                        ...
                        system.query.getGasEstimateAsETH (ewm: eid,
                                                          wid: wid,
                                                          tid: tid,
                                                          from: from,
                                                          to: to,
                ...
                funcSubmitTransaction: { (context, coreEWM, wid, tid, transaction, rid)  in
                    ...
                    if let manager = system.managerBy (eth: eid) {
                        ...
                        system.query.submitTransactionAsETH (ewm: eid,
                                                             wid: wid,
                                                             tid: tid,
                ...
                funcGetTransactions: { (context, coreEWM, address, begBlockNumber, endBlockNumber, rid) in
                    ...
                    if let manager = system.managerBy (eth: eid) {
                        let address = asUTF8String(address!)
                        system.query.getTransactionsAsETH (ewm: eid,
                                                           address: address,
                                                           begBlockNumber: begBlockNumber,
                                                           endBlockNumber: endBlockNumber,
                                                           rid: rid,
                ...
                funcGetLogs: { (context, coreEWM, contract, address, event, begBlockNumber, endBlockNumber, rid) in
                    ...
                    if let manager = system.managerBy (eth: eid) {
                        ...
                        system.query.getLogsAsETH (ewm: eid,
                                                   contract: contract,
                                                   address: address,
                                                   event: event,
                                                   begBlockNumber: begBlockNumber,
                ...
                funcGetBlocks: { (context, coreEWM, address, interests, blockStart, blockStop, rid) in
                    ...
                    if nil !=  system.managerBy (eth: eid) {
                        let address = asUTF8String(address!)
                        system.query.getBlocksAsETH (ewm: eid,
                            address: address,
                            interests: interests,
                ...
                funcGetTokens: { (context, coreEWM, rid) in
                    ...
                    if nil != system.managerBy (eth: eid) {
                        system.query.getTokensAsETH (ewm: eid,
                                                     rid: rid,
                                                     done: { (success: Bool, rid: Int32) in
                ...
                funcGetBlockNumber: { (context, coreEWM, rid) in
                    ...
                    if nil != system.managerBy (eth: eid) {
                        system.query.getBlockNumberAsETH (ewm: eid,
                ...
                funcGetNonce: { (context, coreEWM, address, rid) in
                    ...
                    if nil != system.managerBy (eth: eid) {
                        let address = asUTF8String(address!)
                        system.query.getNonceAsETH (ewm: eid,
                                                    address: address,
...
//
//                if let _ = system.managerBy (eth: eid) {
//                    switch event {
...
//
//                guard let manager  = system.managerBy (eth: eid)
...
//
//                guard let wallet = manager.walletByCoreOrCreate (wid,
...
//
//                guard let wallet = manager.walletByImplOrCreate (WalletImplC.Impl.ethereum(ewm: eid, core: wid),
//                                                                 listener: system.listener,
//                                                                 manager: manager,
//                                                                 unit: unit,
...
//                guard let manager = system.managerBy (eth: eid),
//                    let wallet    = manager.walletBy (impl: WalletImplC.Impl.ethereum(ewm: eid, core: wid)),
//                    let transfer  = wallet.transferByImplOrCreate (TransferImplC.Impl.ethereum (ewm: eid, core: tid),
//                                                                   listener: system.listener,
...
//                guard let manager  = system.managerBy (btc: bid),
//                    let   wallet   = manager.walletBy (impl: WalletImplC.Impl.bitcoin(wid: wid)),
//                    let   transfer = wallet.transferByImplOrCreate (TransferImplC.Impl.bitcoin(wid: wid, tid: tid),
//                                                                    listener: system.listener,
...
//
//                guard let manager = system.managerBy (btc: bid),
//                    let   wallet  = manager.walletByImplOrCreate (WalletImplC.Impl.bitcoin(wid: wid),
//                                                                  listener: system.listener,
//                                                                  manager: manager,
...
//                    guard let transfer = wallet.transferBy (impl: TransferImplC.Impl.bitcoin(wid: wid, tid: event.u.submitted.transaction))
