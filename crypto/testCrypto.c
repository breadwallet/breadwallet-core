...
runCryptoTransferTests (void) {
    ...
    size_t numberOfTests = sizeof (tests) / sizeof (BRCryptoTransferTest);
    for (size_t index = 0; index < numberOfTests; index++) {
        BRCryptoTransferTest *test = &tests[index];
        ...
        BRWalletRegisterTransaction (wid, tid);
        ...
        char *source = cryptoAddressAsString (cryptoTransferGetSourceAddress(transfer));
        char *target = cryptoAddressAsString (cryptoTransferGetTargetAddress(transfer));
